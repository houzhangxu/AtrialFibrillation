...
{   //手术参数控制器
    ...
    public function index(Request $request){
        ...
        $form =  Operation::where("id_card",$id_card)->first();  //根据id查询病人冠心病
        ...
        if($request->isMethod("POST")){
            ...
            if(Operation::where("id_card",$id_card)->count()){
                if(Operation::where("id_card",$id_card)->update($form)){
                    return redirect("/Operation?id_card=".$id_card)->with("result",["code"=>1,"message"=>"修改成功!"]);
                }else{
            ...
            if(Operation::create($form)){
                return redirect("/Operation?id_card=".$id_card)->with("result",["code"=>1,"message"=>"创建成功!"]);
            }else{
