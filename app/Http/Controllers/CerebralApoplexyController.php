...
{   //脑卒中控制器
    ...
    public function index(Request $request){
        ...
        $form =  CerebralApoplexy::where("id_card",$id_card)->first();  //根据id查询病人脑卒中
        ...
        if($request->isMethod("POST")){
            ...
            if(CerebralApoplexy::where("id_card",$id_card)->count()){
                if(CerebralApoplexy::where("id_card",$id_card)->update($form)){
                    return redirect("/CerebralApoplexy?id_card=".$id_card)->with("result",["code"=>1,"message"=>"修改成功!"]);
                }else{
            ...
            if(CerebralApoplexy::create($form)){
                return redirect("/CerebralApoplexy?id_card=".$id_card)->with("result",["code"=>1,"message"=>"创建成功!"]);
            }else{
