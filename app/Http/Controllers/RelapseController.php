...
{   //房颤复发情况控制器
    ...
    public function index(Request $request){
        ...
        $form =  Relapse::where("id_card",$id_card)->first();  //根据id查询病人冠心病
        ...
        if($request->isMethod("POST")){
            ...
            if(Relapse::where("id_card",$id_card)->count()){
                if(Relapse::where("id_card",$id_card)->update($form)){
                    return redirect("/Relapse?id_card=".$id_card)->with("result",["code"=>1,"message"=>"修改成功!"]);
                }else{
            ...
            if(Relapse::create($form)){
                return redirect("/Relapse?id_card=".$id_card)->with("result",["code"=>1,"message"=>"创建成功!"]);
            }else{
